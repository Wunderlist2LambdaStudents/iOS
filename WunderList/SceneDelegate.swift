...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(
        ...
        if AuthService.activeUser == nil {
            ...
            guard let authVC =
                storyboard.instantiateViewController(withIdentifier: "AuthVC") as? LoginViewController else { return }
            authVC.modalPresentationStyle = .fullScreen

            guard let tabBarVC = window?.rootViewController as? UITabBarController,
                let initialView = tabBarVC.viewControllers?[0] as? TodoListViewController else { return }
